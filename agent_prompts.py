...
def data_fetcher_agent_prompt():
        ...
        user = """
            ...
            Reflect on which database, collection(s) fields or sub fields  or transformation of fields you chose and  understand what each field does and its aggregation pipeline e.g users.leaveDate.leave_status: This field indicates whether a leave request has been approved or not
            **Note** : If the user's question involves a time unit (e.g., minutes, hours, years, months) not directly stored in the database,  derive it by converting from related fields and include it as a new field in the output (e.g., convert months to years  or minutes to hours)
            Also formulate a proper approach to get the correct data and  the final result.
            
            Now here comes the main part after you figured out everthing thing:
